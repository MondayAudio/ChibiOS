...
Change History
==============
26.11.18	Modify I2C slave handling to always set idle mode and continue when NAK received. (Resolves some sequencing problems)
			Only ever send STOP in master mode
			Process received STOP (slave mode) before address match check denoting new transaction
23.11.18    Additional debug logger cases
14.11.18	Report slave errors in main error flag location
 ...
 *  0xcd - clearing down due to NAK received (probably not valid, except in master mode maybe)
 *  0xd0 - Slave error being signalled via 'exception' callback
 *  0xd1 - Slave error due to timeout
 *  0xe0 - Slave normal transaction end, moved to idle state
 *  0xe1 - Master normal transaction end, moved to idle state
*/
#define QEVENTS 32
 ...
 * @notapi
 */
static inline void reportSlaveError(I2CDriver *i2cp, i2cflags_t errFlags) {
  ...
  i2cp->errors = errFlags | I2C_SLAVE_ERROR;
  if (xfer->exception)
      xfer->exception(i2cp);
  i2cp->mode = i2cIdle;
...
static void slaveTimeExpired(void *i2cv) {
  ...
  if (i2cp->mode < i2cIsMaster)
  {
    ...
    reportSlaveError(i2cp, I2C_SLAVE_TIMEOUT);
    qEvt(0xd1, 0);
  }
}
...
//  uint32_t tempIsr;
  ...
  if ((isr & I2C_ISR_NACKF) != 0U)
  {
    ...
    dp->CR1 &= ~(I2C_CR1_TCIE | I2C_CR1_STOPIE);
    // TXIE and RXIE already cleared
    ...
    else
    {
      ...
      /* Transaction finished, send the STOP. Only if master */
...
//      _i2c_wakeup_isr(i2cp);          // This is a normal completion
  ...
  // If receive callback enabled, always transfer using interrupts
	...
	if ((isr & I2C_ISR_STOPF) != 0U)
	{
		...
		*
		*	Do this before doing another address match check
		*/
		qEvt(0x05, 0);
	...
	uint8_t abort = 0;
	/* Check for address match - if so, we're slave. Start new transaction */
	if ((isr & I2C_ISR_ADDR) != 0U)
	{
        ...
        switch (i2cp->mode) {
            ...
            default:
                ...
                stopTimer(i2cp);
                reportSlaveError(i2cp, I2C_SLAVE_STATE_ERROR);                 // This clears down to idle
                abort = 1;
            }
        if (!abort)
        {
...
static void i2c_lld_serve_error_interrupt(I2CDriver *i2cp, uint32_t isr) {
    ...
    uint32_t errFlags = 0;				// USed to accumulate errors
  ...
  if (isr & I2C_ISR_BERR)
    errFlags |= I2C_BUS_ERROR;

  if (isr & I2C_ISR_ARLO)
    errFlags |= I2C_ARBITRATION_LOST;

  if (isr & I2C_ISR_OVR)
    errFlags |= I2C_OVERRUN;

  if (isr & I2C_ISR_TIMEOUT)
    errFlags |= I2C_TIMEOUT;

#if HAL_USE_I2C_SLAVE
    // In slave mode, just clock errors and return
	if (i2cp->mode < i2cIsMaster)
	{
	  reportSlaveError(i2cp, I2C_SLAVE_FLAGS_ERROR | errFlags);           // This clears down to idle
	  return;
	}
#endif


  i2cp->errors |= errFlags;
